...
@section('content')
    <div class="container">
        @if (isset($tipo))
            <h5 class="text-center">Resultados /
                {{ ucwords(str_replace('_', ' ', $tipo)) }} </h5>
        @endif        
        <div class="row">
            <div class="col-sm-12">
                    ...
                    <tbody>
                        @foreach ($proyectos as $item)
                                ...
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>{{ $item->folio }}</td>
                                    <td>{{ ucfirst(mb_strtolower($item->titulo_proyecto, 'UTF-8')) }}</td>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            var table = $('#myTable').DataTable({
                ...
                buttons: [{
                        extend: 'excelHtml5',
                        title: 'Proyectos de investigación resultados ' + "<?php echo $tipo; ?>" ,
                        exportOptions: {
